...
export default function AboutContactSection({ profile }: Props) {
  ...
  const aboutX = useTransform(scrollYProgress, [0, 0.42, 0.55, 1], [0, 0, -90, -140]);
  const aboutOpacity = useTransform(scrollYProgress, [0, 0.32, 0.48, 0.6], [1, 1, 0.25, 0]);
  const contactX = useTransform(scrollYProgress, [0, 0.36, 0.62, 1], [120, 70, 0, 0]);
  const contactOpacity = useTransform(scrollYProgress, [0, 0.22, 0.42, 0.55], [0, 0.1, 1, 1]);
  const imageScale = useTransform(scrollYProgress, [0, 0.5, 1], [1.02, 1.08, 1.14]);
  const imageY = useTransform(scrollYProgress, [0, 0.5, 1], [0, -8, -22]);

  return (
    <section ref={sectionRef} className="relative h-[155vh] min-h-[980px]">
      <div className="sticky top-16 h-[82vh] min-h-[640px] overflow-hidden rounded-3xl border border-white/15 bg-[#060b16] p-4 sm:p-5">
        <motion.div style={{ x: aboutX, opacity: aboutOpacity }} className="absolute inset-0 p-4 sm:p-5">
          <div className="relative h-full overflow-hidden rounded-3xl border border-white/15 bg-[#060b16] p-5 sm:p-6">
            <motion.div
        ...
        <motion.div style={{ x: contactX, opacity: contactOpacity }} className="absolute inset-0 p-4 sm:p-5">
          <div className="relative h-full overflow-hidden rounded-3xl border border-blue-400/30 bg-gradient-to-br from-slate-950 via-blue-950 to-violet-800 p-5 shadow-[0_0_44px_rgba(59,130,246,0.2)] sm:p-6">
            <motion.div
              animate={{ y: [0, -10, 0] }}
